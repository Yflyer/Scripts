...
for i in `ls *.fastq`;
do
    cutadapt -g CCAGCASCYGCGGTAATTCC -o ptrim_${i} -m 200 -M 270 --trimmed-only --max-n 0 -j 8 ${i} >> ../../ptrim_r1_report_txt
    rm ${i}
done
...
for i in `ls *.fastq`;
do
    cutadapt -g ACTTTCGTTCTTGATYRA -o ptrim_${i} -m 200 -M 270 --trimmed-only --max-n 0 -j 8 ${i} >> ../../ptrim_r2_report_txt
    rm ${i}
done
